...
func InstallForConnection(ctx context.Context, tx *gorm.DB, orgID, userID uuid.UUID, provider string) error {
	...
	if err := tx.WithContext(ctx).
		...
		return fmt.Errorf("load plugins for connection: %w", err)
	}
	...
	for _, pluginID := range pluginIDs {
		...
		if err := tx.WithContext(ctx).Clauses(clause.OnConflict{DoNothing: true}).Create(&install).Error; err != nil {
			return fmt.Errorf("install connection plugin: %w", err)
		}
