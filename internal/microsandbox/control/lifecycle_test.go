...
func TestIdleLifecycleStopsPolicyEnabledSandbox(t *testing.T) {
	...
	var stopCalls int32
	runnerServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/v1/sandboxes/sbx_idle/connections" {
			_ = json.NewEncoder(w).Encode(map[string]int{"active_connections": 0})
			return
		}
...
func TestIdleLifecycleSkipsBusyRuntime(t *testing.T) {
	...
	if err := db.Create(&model.Sandbox{
		...
		ImageRef: "image:test", Status: model.SandboxStatusRunning, CPU: 1, MemoryMB: 2048, DiskGB: 40,
		AutoSleepAfterSeconds: 300, LastGatewayActivityAt: &old, LastRuntimeActivityAt: &now, RuntimeBusy: true, CreatedAt: old, SleepAfterAt: &due,
	}).Error; err != nil {
		t.Fatal(err)
	}
...
func TestIdleLifecycleSkipsActiveConnections(t *testing.T) {
	const runnerToken = "runner-token"
	var stopCalls int32
	runnerServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/v1/sandboxes/sbx_connected/connections":
			_ = json.NewEncoder(w).Encode(map[string]int{"active_connections": 2})
		case "/v1/sandboxes/sbx_connected/stop":
			atomic.AddInt32(&stopCalls, 1)
			_ = json.NewEncoder(w).Encode(map[string]string{"status": "stopped"})
		default:
			t.Fatalf("runner path = %s", r.URL.Path)
	...
	if err := db.Create(&model.Sandbox{
		ID: "sbx_connected", OrgID: "org_1", RunnerID: "runner-1", Name: "test",
		ImageRef: "image:test", Status: model.SandboxStatusRunning, CPU: 1, MemoryMB: 2048, DiskGB: 40,
		AutoSleepAfterSeconds: 300, LastGatewayActivityAt: &old, CreatedAt: old, SleepAfterAt: &due,
	}).Error; err != nil {
		t.Fatal(err)
	}
	if err := db.Create(&model.SandboxPort{ID: "port-1", SandboxID: "sbx_connected", GuestPort: 3000, HostPort: 43000, Protocol: "http"}).Error; err != nil {
		t.Fatal(err)
	}
	...
	s.stopIdleSandboxes(context.Background())
	if got := atomic.LoadInt32(&stopCalls); got != 0 {
		t.Fatalf("stop calls = %d, want 0 while connections are active", got)
	}
	var sb model.Sandbox
	if err := db.First(&sb, "id = ?", "sbx_connected").Error; err != nil {
		t.Fatal(err)
	}
	if sb.Status != model.SandboxStatusRunning || sb.SleepAfterAt == nil || !sb.SleepAfterAt.After(time.Now()) {
		t.Fatalf("sandbox not extended: %+v", sb)
	}
}
