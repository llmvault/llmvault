...
// Tool names. list_org_plugins is read-only; create_agent and update_agent are
// privileged, opt-in mutating tools gated per calling agent.
const (
...
// NewToolsFunc returns the agent-builder ToolsFunc. It registers
// list_org_plugins, create_agent, and update_agent on the MCP server ONLY when
// the calling agent is permitted (see agentBuilderEnabled): the org's default
// Hivy agent by default, or any agent whose McpToolFilter.Allow explicitly
// contains create_agent/update_agent. frontendURL is used to build the agent
// URL in tool responses.
