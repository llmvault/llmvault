...
func (o *Orchestrator) StartEmployeeSandbox(ctx context.Context, sb *model.Sandbox) error {
	...
	if err := o.pushEmployeeRuntimeConfig(ctx, sb, "start"); err != nil {
		return err
	}
	return nil
}
...
func (o *Orchestrator) RestartEmployeeSandbox(ctx context.Context, sb *model.Sandbox) error {
	...
	if restarter, ok := o.provider.(RestartableProvider); ok {
		if err := restarter.RestartSandbox(ctx, sb.ExternalID); err != nil {
			return fmt.Errorf("restarting employee sandbox %s: %w", sb.ID, err)
		}
		if err := o.RefreshEmployeeSandboxURL(ctx, sb); err != nil {
			return err
		}
		...
		if err := o.pushEmployeeRuntimeConfig(ctx, sb, "restart"); err != nil {
			return err
		}
		return nil
	}
