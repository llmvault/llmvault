...
func TestRunSandboxLifecycle_SkipsPersistentAgents(t *testing.T) {
	orch, provider, db := setupOrchestrator(t)
	org := createTestOrg(t, db)
	cred := createTestCred(t, db, org.ID)

	agentAgent := newAgentWithHarness(t, db, org.ID, cred.ID, "agent-sandbox")
	claudeAgent := newAgentWithHarness(t, db, org.ID, cred.ID, "claude")

	idleAt := time.Now().Add(-30 * time.Minute)
	agentSb := newRunningSandbox(t, db, &org.ID, &agentAgent.ID, "ext-agent", idleAt)
	claudeSb := newRunningSandbox(t, db, &org.ID, &claudeAgent.ID, "ext-claude", idleAt)
	provider.registerSandbox("ext-agent", StatusRunning)
	provider.registerSandbox("ext-claude", StatusRunning)

	orch.RunSandboxLifecycle(context.Background())

	var agentAfter, claudeAfter model.Sandbox
	db.Where("id = ?", agentSb.ID).First(&agentAfter)
	db.Where("id = ?", claudeSb.ID).First(&claudeAfter)

	if agentAfter.Status != string(StatusRunning) {
		t.Errorf("agent sandbox status = %q, want running (excluded from idle stop)", agentAfter.Status)
	}
...
func TestRunSandboxLifecycle_SkipsPersistentAgentsFromArchive(t *testing.T) {
	orch, provider, db := setupOrchestrator(t)
	org := createTestOrg(t, db)
	cred := createTestCred(t, db, org.ID)
	...
	agentSb := newStoppedSandbox(t, db, &org.ID, &agentAgent.ID, "ext-e-stop", stoppedAt)
	...
	if agentAfter.Status != string(StatusStopped) {
		t.Errorf("agent sandbox status = %q, want stopped (excluded from archive)", agentAfter.Status)
	}
...
func newAgentWithHarness(t *testing.T, db *gorm.DB, orgID, credID uuid.UUID, harness string) model.Agent {
	...
	a := model.Agent{
		...
		Harness: harness, IsManaged: harness == "agent-sandbox",
		Status: "active",
	}
