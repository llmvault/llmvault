...
func TestDriverCreateDeveloperWarmSlotDisablesDockerAutostart(t *testing.T) {
	var createReq map[string]any
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch {
		case r.Method == http.MethodPost && r.URL.Path == "/v1/sandboxes":
			...
			_ = json.NewEncoder(w).Encode(map[string]any{"ID": "sbx_warm_developer", "Status": "running"})
		case r.Method == http.MethodPost && r.URL.Path == "/v1/sandboxes/sbx_warm_developer/runtime-endpoints":
			_ = json.NewEncoder(w).Encode(map[string]string{"url": "https://7080-sbx_warm_developer.preview.test"})
		default:
			http.NotFound(w, r)
		}
	}))
	...
	if _, err := driver.CreateWarmSlot(context.Background(), sandbox.WarmSlotCreateOpts{
		Name:          "hivy-agent-developer-small-warm-test",
		Mode:          "agent",
		ImageKind:     "developer",
		SandboxSize:   "small",
		RuntimeImage:  "ghcr.io/usehivy/hivy-sandboxes-runtime-developers:v3.8.0-amd64",
		RuntimePort:   sandbox.AgentSandboxPort,
		CPU:           1,
		Memory:        2,
		Disk:          10,
