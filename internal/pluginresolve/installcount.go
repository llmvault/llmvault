...
// teams (of orgs with an active install) plus default agents for
// default-agent-install plugins. The count is derived with a single set-based
// query rather than a per-agent fan-out, which deadlocked under concurrent load.
...
func effectiveAgentCountForPlugin(ctx context.Context, tx *gorm.DB, pluginID uuid.UUID) (int64, error) {
	...
	if err := tx.WithContext(ctx).
		...
		Where("(tp.plugin_id IS NOT NULL OR (? AND agents.is_default))", PluginDefaultAgentInstall(plugin)).
		Distinct("agents.id").
		Count(&count).Error; err != nil {
