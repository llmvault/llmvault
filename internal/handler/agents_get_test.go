...
func (h *agentHarness) getAgent(t *testing.T, m orgWithMember, agentID string) *httptest.ResponseRecorder {
	t.Helper()
...
func TestIntegration_AgentsGet_HappyPath_LoadsAgentAndSandbox(t *testing.T) {
	h := newAgentHarness(t)
	...
	rr := h.getAgent(t, m, emp.ID.String())
	...
	if _, exposed := item["profiles"]; exposed {
		t.Fatalf("agent response exposed removed profiles field: %#v", item["profiles"])
	}
...
func TestIntegration_AgentsGet_EnsuresMissingMemoryBank(t *testing.T) {
	h := newAgentHarness(t)
	h.platformCredCleanup(t)
	m := h.createOrg(t)
	emp := h.seedAgentAgent(t, m)
...
func TestIntegration_AgentsGet_ReportsSandboxUpgradeAvailability(t *testing.T) {
	h := newAgentHarness(t)
	h.platformCredCleanup(t)
	m := h.createOrg(t)
	emp := h.seedAgentAgent(t, m)
	...
	outdatedSnapshot := "older-agent-sandbox"
...
func TestIntegration_AgentsGet_ScopedToOrg(t *testing.T) {
	h := newAgentHarness(t)
	h.platformCredCleanup(t)
	owner := h.createOrg(t)
	stranger := h.createOrg(t)
	emp := h.seedAgentAgent(t, owner)

	rr := h.getAgent(t, stranger, emp.ID.String())
