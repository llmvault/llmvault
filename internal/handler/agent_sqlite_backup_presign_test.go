...
func (h *sqliteBackupHarness) presignBackup(t *testing.T, body any, runtimeSecret string) *httptest.ResponseRecorder {
	...
	req := httptest.NewRequest(http.MethodPost, "/internal/agents/"+h.agentID.String()+"/sqlite-backup/presign", payload)
	if runtimeSecret != "" {
...
func (h *sqliteBackupHarness) confirmBackup(t *testing.T, body any, runtimeSecret string) *httptest.ResponseRecorder {
	...
	req := httptest.NewRequest(http.MethodPost, "/internal/agents/"+h.agentID.String()+"/sqlite-backup/confirm", bytes.NewReader(raw))
	if runtimeSecret != "" {
		req.Header.Set("Authorization", "Bearer "+runtimeSecret)
	}
	req.Header.Set("Content-Type", "application/json")
...
func TestAgentSQLiteBackup_PresignAllowsDirectStorageUploadAndConfirm(t *testing.T) {
...
func TestAgentSQLiteBackup_ConfirmRejectsCrossAgentKey(t *testing.T) {
	h := newSQLiteBackupHarness(t, 1024*1024, true)
	rr := h.confirmBackup(t, map[string]any{
		"key":   fmt.Sprintf("agent-sqlite-backups/%s/%s/latest.db.gz", uuid.New(), uuid.New()),
		"bytes": 10,
	}, h.runtimeSecret)
...
func TestAgentSQLiteBackup_PresignRejectsOversizedBackupHint(t *testing.T) {
