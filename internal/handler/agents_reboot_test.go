...
func TestAgentHandler_RebootSandboxRestartsAndSyncsRuntimeConfig(t *testing.T) {
	h := newAgentHarness(t)
	m := h.createOrg(t)
	...
	rr := h.rebootAgentSandbox(t, m, agent.ID, "admin")
	...
	var links int64
	if err := h.db.Model(&model.AgentSkill{}).
		Where("agent_id = ? AND skill_id = ?", agent.ID, drive.ID).
		Count(&links).Error; err != nil {
		t.Fatalf("count drive link: %v", err)
	}
	if links != 1 {
		t.Fatalf("drive agent skill links = %d, want 1", links)
	}
	...
	if resp.SandboxID != sb.ID.String() || resp.Agent.ID != agent.ID.String() || resp.Sync.Applied != 1 {
...
func TestAgentHandler_RebootSandboxEnsuresMissingMemoryBank(t *testing.T) {
	h := newAgentHarness(t)
	m := h.createOrg(t)
	agent := h.seedAgentAgent(t, m)
...
func TestAgentHandler_RebootSandboxRequiresAdmin(t *testing.T) {
	h := newAgentHarness(t)
	m := h.createOrgWithRole(t, "member")
	agent := h.seedAgentAgent(t, m)
	h.seedSandbox(t, m, agent.ID)

	rr := h.rebootAgentSandbox(t, m, agent.ID, "member")
