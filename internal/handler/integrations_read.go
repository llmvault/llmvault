...
func (h *IntegrationHandler) List(w http.ResponseWriter, r *http.Request) {
	...
	q := h.db.WithContext(r.Context()).Where("deleted_at IS NULL")
...
func (h *IntegrationHandler) Get(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.db.WithContext(r.Context()).Where("id = ? AND deleted_at IS NULL", integID).First(&integ).Error; err != nil {
...
// @Tags integrations
// @Produce json
// @Success 200 {object} paginatedResponse[integrationAvailableResponse]
// @Security BearerAuth
// @Router /v1/integrations/available [get]
func (h *IntegrationHandler) ListAvailable(w http.ResponseWriter, r *http.Request) {
	var integrations []model.Integration
	if err := h.db.WithContext(r.Context()).Where("deleted_at IS NULL").Order("created_at ASC").Find(&integrations).Error; err != nil {
		writeJSON(w, http.StatusInternalServerError, map[string]string{"error": "failed to list integrations"})
		return
	}
	...
	writeJSON(w, http.StatusOK, struct {
		Data []integrationAvailableResponse `json:"data"`
	}{Data: resp})
}
