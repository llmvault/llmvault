...
func TestAgentHandlerSyncOrgHivyAgent_CreatesSandboxAndPushesConfig(t *testing.T) {
	h := newAgentHarness(t)
	m := h.createOrg(t)
	agent := h.seedAgentAgent(t, m)

	if err := h.handler.SyncOrgHivyAgent(t.Context(), m.org.ID); err != nil {
		t.Fatalf("SyncOrgHivyAgent: %v", err)
	}
	...
	if err := h.db.Where("org_id = ? AND agent_id = ? AND status = ?", m.org.ID, agent.ID, "running").First(&sb).Error; err != nil {
...
func TestAgentHandlerSyncOrgHivyAgent_PushesCustomOrgEnvironmentVariables(t *testing.T) {
	h := newAgentHarness(t)
	m := h.createOrg(t)
	agent := h.seedAgentAgent(t, m)
	h.setRuntimeEnvVars(t, agent.ID, map[string]string{
		"HIVY_ORG_STRIPE_API_KEY": "sk_test_agent_sync",
	})
	...
	if env["HIVY_ORG_STRIPE_API_KEY"] != "sk_test_agent_sync" {
		t.Fatalf("runtime env HIVY_ORG_STRIPE_API_KEY = %q, want sk_test_agent_sync", env["HIVY_ORG_STRIPE_API_KEY"])
	}
}
