...
func TestAgentHandler_ListModelsReturnsAgentOpenRouterAllowlist(t *testing.T) {
	db := connectTestDB(t)
	cred := model.Credential{
		Label:        "test-openrouter-agent-models",
	...
	req := httptest.NewRequest(http.MethodGet, "/v1/agents/models", nil)
...
func TestAgentHandler_UpdateModelPersistsAndPushesRuntimeConfig(t *testing.T) {
	h := newAgentHarness(t)
	m := h.createOrg(t)
	agent := h.seedAgentAgent(t, m)
	h.seedSandbox(t, m, agent.ID)

	rr := h.patchAgentModel(t, m, agent.ID, map[string]any{
	...
	if updated.Model != "step-3.7-flash" {
		t.Fatalf("agent model = %q, want step-3.7-flash", updated.Model)
	}
	...
	if resp.Agent.Model != "step-3.7-flash" || resp.Sync.Applied != 1 {
		t.Fatalf("response = %#v", resp)
	}
}

func TestAgentHandler_UpdateModelRejectsNonAgentModel(t *testing.T) {
	h := newAgentHarness(t)
	m := h.createOrg(t)
	agent := h.seedAgentAgent(t, m)
...
func TestAgentHandler_UpdateModelRequiresAdmin(t *testing.T) {
	h := newAgentHarness(t)
