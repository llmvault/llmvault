...
func TestAgentOutboundWebhookBatch_IngestsCoalescedStreamWithoutPerDeltaRows(t *testing.T) {
	...
	agent := model.Agent{OrgID: &org.ID, Name: "Higu", Model: "test", IsManaged: true}
	...
	r := chi.NewRouter()
	r.Post("/internal/webhooks/agent/{sandboxID}/batch", h.HandleBatch)
	...
	body := gzipNDJSON(t,
		agentOutboundEvent{EventType: "agent.stream.thinking", Payload: mustJSON(t, streamPayload), At: time.Now().UTC()},
		agentOutboundEvent{EventType: "agent.message.sent", Payload: mustJSON(t, finalPayload), At: time.Now().UTC()},
	)
	req := httptest.NewRequest(http.MethodPost, "/internal/webhooks/agent/"+sandbox.ID.String()+"/batch", bytes.NewReader(body))
	...
	var stored model.AgentSessionEvent
...
func TestAgentOutboundWebhookBatch_RejectsBadSignature(t *testing.T) {
	db := connectAgentSkillSyncTestDB(t)
	encKey := outboundWebhookTestSymmetricKey(t)
	org := model.Org{Name: "batch-webhook-" + uuid.NewString(), RateLimit: 1000, Active: true}
	if err := db.Create(&org).Error; err != nil {
	...
	sandbox := model.Sandbox{OrgID: &org.ID, AgentID: &agent.ID, EncryptedRuntimeSecret: encryptedKey, Status: "running"}
	if err := db.Create(&sandbox).Error; err != nil {
		t.Fatalf("create sandbox: %v", err)
	}
	h := NewAgentOutboundWebhookHandler(db, encKey, nil)
	r := chi.NewRouter()
	r.Post("/internal/webhooks/agent/{sandboxID}/batch", h.HandleBatch)
	body := gzipNDJSON(t, agentOutboundEvent{EventType: "agent.stream.token", Payload: mustJSON(t, map[string]any{"session_id": "s"}), At: time.Now().UTC()})
	req := httptest.NewRequest(http.MethodPost, "/internal/webhooks/agent/"+sandbox.ID.String()+"/batch", bytes.NewReader(body))
	req.Header.Set("Content-Encoding", "gzip")
...
func gzipNDJSON(t *testing.T, events ...agentOutboundEvent) []byte {
	t.Helper()
