...
func (h *AgentOutboundWebhookHandler) HandleBatch(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	...
	if err != nil {
		captureAgentWebhookIngest(ctx, "read_batch_body", sb, nil, "", "", err)
	...
	if strings.EqualFold(r.Header.Get("Content-Encoding"), "gzip") {
		...
		if err != nil {
			captureAgentWebhookIngest(ctx, "decode_batch_gzip", sb, nil, "", "", err)
	...
	for scanner.Scan() {
		...
		if err := json.Unmarshal([]byte(line), &event); err != nil {
			captureAgentWebhookIngest(ctx, "decode_batch_event", sb, nil, "", "", err)
		...
		if err := h.storeAndMaybeEnqueue(ctx, sb, &event); err != nil {
			...
			captureAgentWebhookIngest(ctx, "store_outbound_batch_event", sb, &event, "", "", err)
	...
	if err := scanner.Err(); err != nil {
		captureAgentWebhookIngest(ctx, "read_batch_line", sb, nil, "", "", err)
	...
	if err := h.db.WithContext(ctx).Model(sb).Update("last_active_at", h.now()).Error; err != nil {
		captureAgentWebhookIngest(ctx, "update_last_active_batch", sb, nil, "", "", err)
	}
