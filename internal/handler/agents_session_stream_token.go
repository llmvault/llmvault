...
func (h *AgentHandler) signedWebStreamURL(agentID, sessionID uuid.UUID, streamID string) (string, error) {
	...
	return fmt.Sprintf("/v1/agents/%s/sessions/%s/streams/%s?token=%s", agentID, sessionID, streamID, token), nil
}
...
func (h *AgentHandler) signWebStreamToken(sessionID uuid.UUID, streamID string, expiresAt int64) (string, error) {
...
func (h *AgentHandler) verifyWebStreamToken(sessionID uuid.UUID, streamID, token string) bool {
...
func (h *AgentHandler) directWebStreamURLs(sandbox model.Sandbox, delivery *agentruntime.HTTPMessageResponse) (string, string) {
