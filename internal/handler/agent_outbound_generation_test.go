...
func TestAgentOutboundModelUsage_WritesGenerationAndSkipsMemoryEvent(t *testing.T) {
	...
	sandbox := model.Sandbox{OrgID: &org.ID, AgentID: &agent.ID, ExternalID: "runtime-usage-sandbox", RuntimeURL: "http://localhost:7080", EncryptedRuntimeSecret: []byte("key"), Status: "running"}
	...
	db.Model(&model.AgentSessionEvent{}).Where("sandbox_id = ?", sandbox.ID).Count(&eventCount)
	if eventCount != 0 {
