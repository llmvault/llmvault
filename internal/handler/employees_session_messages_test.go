...
func (h *employeeHarness) sendSessionMessage(t *testing.T, m orgWithMember, agentID uuid.UUID, body any) *httptest.ResponseRecorder {
	t.Helper()
	buf := new(bytes.Buffer)
	_ = json.NewEncoder(buf).Encode(body)
	req := httptest.NewRequest(http.MethodPost, "/v1/employees/"+agentID.String()+"/sessions/messages", buf)
...
func (h *employeeHarness) streamSession(t *testing.T, m orgWithMember, streamURL string) *httptest.ResponseRecorder {
	t.Helper()
...
func TestEmployeeHandler_SendSessionMessageCreatesWebSessionAndRuntimeTurn(t *testing.T) {
	...
	if out.EmployeeSessionID == "" || out.RuntimeSessionID == "" || out.ResponseStreamURL == "" {
	...
	if session.Source != "web" || session.EmployeeID != agent.ID || session.OrgID != m.org.ID {
	...
	if !ok || raw["source"] != "web" || raw["employee_session_id"] != session.ID.String() {
...
func TestEmployeeHandler_SendSessionMessageContinuesExistingWebSession(t *testing.T) {
	...
	second := h.sendSessionMessage(t, m, agent.ID, map[string]any{
		"session_id": firstOut.EmployeeSessionID,
		"text":       "Second",
	})
	...
	if secondOut.Created || secondOut.EmployeeSessionID != firstOut.EmployeeSessionID {
		t.Fatalf("second response = %#v, first = %#v", secondOut, firstOut)
	}

	var count int64
	h.db.Model(&model.EmployeeSession{}).Where("org_id = ? AND employee_id = ? AND source = ?", m.org.ID, agent.ID, "web").Count(&count)
	if count != 1 {
...
func TestEmployeeHandler_StreamSessionProxiesSignedRuntimeSSE(t *testing.T) {
	h := newEmployeeHarness(t)
	m := h.createOrgWithRole(t, "member")
	agent := h.seedEmployeeAgent(t, m)
	h.seedSandbox(t, m, agent.ID)
