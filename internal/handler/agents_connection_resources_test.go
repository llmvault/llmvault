...
func TestAgentHandler_UpdateConnectionResourcesStoresOnAgentAndQueuesGitHubClone(t *testing.T) {
	...
	t.Cleanup(func() {
		...
		db.Where("1=1").Delete(&model.Agent{})
	})
	...
	h.SetEnqueuer(enq)
	r := chi.NewRouter()
	r.Put("/v1/agents/{id}/connections/{connectionID}/resources", h.UpdateConnectionResources)
	...
	req := httptest.NewRequest(http.MethodPut,
		"/v1/agents/"+agent.ID.String()+"/connections/"+conn.ID.String()+"/resources",
