...
func newSQLiteBackupHarness(t *testing.T, maxBytes int64, isAgent bool) *sqliteBackupHarness {
	t.Helper()
	s3 := newRealS3Client(t)
	return newSQLiteBackupHarnessWithStreamer(t, maxBytes, isAgent, s3, s3)
}
...
func newSQLiteBackupHarnessWithStreamer(t *testing.T, maxBytes int64, isAgent bool, streamer sqliteBackupStreamer, s3 *storage.S3Client) *sqliteBackupHarness {
	t.Helper()
	db := connectTestDB(t)
	...
	h := handler.NewAgentSQLiteBackupHandler(db, streamer, encKey, maxBytes)
	r := chi.NewRouter()
	r.Put("/internal/agents/{agentID}/sqlite-backup", h.Upload)
	r.Post("/internal/agents/{agentID}/sqlite-backup/presign", h.Presign)
	r.Post("/internal/agents/{agentID}/sqlite-backup/confirm", h.Confirm)
...
func (h *sqliteBackupHarness) uploadBackup(t *testing.T, body []byte, runtimeSecret string) *httptest.ResponseRecorder {
	t.Helper()
	return h.uploadBackupPath(t, "/internal/agents/"+h.agentID.String()+"/sqlite-backup", body, runtimeSecret)
}
...
func (h *sqliteBackupHarness) backupKey() string {
	return fmt.Sprintf("agent-sqlite-backups/%s/%s/latest.db.gz", h.orgID, h.agentID)
}

func (h *sqliteBackupHarness) upgradeBackupKey(upgradeID uuid.UUID) string {
	return fmt.Sprintf("agent-sqlite-backups/%s/%s/upgrades/%s.db.gz", h.orgID, h.agentID, upgradeID)
}
...
func TestAgentSQLiteBackup_UploadsFixedLatestObject(t *testing.T) {
...
func TestAgentSQLiteBackup_SecondUploadOverwritesSameKey(t *testing.T) {
	h := newSQLiteBackupHarness(t, 1024*1024, true)
...
func TestAgentSQLiteBackup_UpgradeIDWritesImmutableUpgradeKey(t *testing.T) {
	h := newSQLiteBackupHarness(t, 1024*1024, true)
	upgradeID := uuid.New()
	...
	path := "/internal/agents/" + h.agentID.String() + "/sqlite-backup?upgrade_id=" + upgradeID.String()
...
func TestAgentSQLiteBackup_InvalidBearer(t *testing.T) {
	h := newSQLiteBackupHarness(t, 1024*1024, true)
...
func TestAgentSQLiteBackup_UnknownAgentRejected(t *testing.T) {
	h := newSQLiteBackupHarness(t, 1024*1024, true)
	path := "/internal/agents/" + uuid.NewString() + "/sqlite-backup"
...
func TestAgentSQLiteBackup_OversizedBodyRejected(t *testing.T) {
