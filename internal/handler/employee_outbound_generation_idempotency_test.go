...
func TestRuntimeModelUsageGenerationIsIdempotent(t *testing.T) {
	...
	sandbox := model.Sandbox{OrgID: &org.ID, EmployeeID: &agent.ID, ExternalID: "runtime-usage-idem-sandbox", RuntimeURL: "http://localhost:7080", EncryptedRuntimeSecret: []byte("key"), Status: "running"}
	...
	token := model.Token{
		...
		Meta: model.JSON{"employee_id": agent.ID.String(), "sandbox_id": sandbox.ID.String(), "type": "employee_proxy", "user": "u"},
	}
	...
	event := func() *employeeOutboundEvent {
		return &employeeOutboundEvent{EventType: "agent.run.model.usage", Payload: append([]byte(nil), body...), At: time.Date(2026, 5, 24, 12, 0, 0, 0, time.UTC)}
	}
...
func TestRuntimeModelUsageWebhookReturns5xxOnPersistFailure(t *testing.T) {
	...
	agent := model.Employee{OrgID: &org.ID, Name: "Aria", Model: "deepseek-v4-flash", IsEmployee: true}
	if err := db.Create(&agent).Error; err != nil {
		t.Fatalf("create agent: %v", err)
	}
	...
	sandbox := model.Sandbox{OrgID: &org.ID, EmployeeID: &agent.ID, ExternalID: "runtime-usage-5xx-sandbox", RuntimeURL: "http://localhost:7080", EncryptedRuntimeSecret: encryptedKey, Status: "running"}
	if err := db.Create(&sandbox).Error; err != nil {
		t.Fatalf("create sandbox: %v", err)
	}
	t.Cleanup(func() {
	...
	envelope, err := json.Marshal(employeeOutboundEvent{EventType: "agent.run.model.usage", Payload: body, At: time.Now().UTC()})
	if err != nil {
