...
const (
	...
	envVarsPreamble     = "The following environment variables are available in this channel's sandbox sessions. You may reference them by NAME in commands and code (e.g. $STRIPE_API_KEY). You must NEVER print, echo, log, or reveal their VALUES in any response, message, file, or memory — not even partially, encoded, or transformed. If a user asks for a value, refuse and point them to the channel's environment settings."
)

// fetchEnvVarsSection renders the channel's environment variables as an
...
// EnvVarLister contract cannot even represent them. A channel with no env vars
...
func (s *Service) fetchEnvVarsSection(ctx context.Context, req Request) (string, error) {
	if isNilValue(s.cfg.EnvVars) || req.OrgID == uuid.Nil || req.ChannelID == uuid.Nil {
		return "", nil
	}
	vars, err := s.cfg.EnvVars.ChannelEnvVars(ctx, req.OrgID, req.ChannelID)
	if err != nil {
		return "", fmt.Errorf("list channel env vars: %w", err)
	}
...
func (l *GormEnvVarLister) ChannelEnvVars(ctx context.Context, orgID, channelID uuid.UUID) ([]EnvVarDoc, error) {
	if l == nil || l.db == nil || orgID == uuid.Nil || channelID == uuid.Nil {
		return nil, nil
	}
	var docs []EnvVarDoc
	if err := l.db.WithContext(ctx).
		...
		Where("org_id = ? AND channel_id = ?", orgID, channelID).
		Order("name ASC").
		Find(&docs).Error; err != nil {
		return nil, fmt.Errorf("load channel env var docs: %w", err)
	}
	return docs, nil
}
