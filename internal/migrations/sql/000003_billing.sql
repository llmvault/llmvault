...
CREATE UNIQUE INDEX idx_credit_ledger_entries_idem ON credit_ledger_entries USING btree (org_id, reason, ref_type, ref_id) WHERE ref_id <> '';

CREATE INDEX idx_plans_provider ON plans USING btree (provider);

CREATE INDEX idx_plans_visible ON plans USING btree (visible);
...
CREATE INDEX idx_tu_org_agent ON tool_usages USING btree (agent_id);
