...
func productionMemorySessions() []productionSession {
	return []productionSession{
		...
		{user: "Maya", text: "The GitHub repo picker modal feels weird. It should match the existing setup selection pattern, not look like a settings table.", tools: []string{"read_file: inspected setup choice card usage", "edit_file: changed repository picker to choice-card layout"}, reply: "Updated to use the existing choice-card pattern. Private repos show a lock icon.", noise: []string{"hmm", "closer"}},
		...
		{user: "Maya", text: "This workspace setup screen is stuck when org creation returns duplicate name. It shouldn't pretend we're still launching.", tools: []string{"read_file: checked setup mutation", "edit_file: surfaced create-org error state"}, reply: "Fixed. Duplicate org-name errors now stop the launch state and show the API error.", noise: []string{"nice"}},
		...
		{user: "Daniel", text: "Can you run browser tests for the new topbar? I don't want this thread stuck running Playwright for ten minutes.", tools: []string{"subagent_task: started topbar browser regression task", "check_subagent_task_status: checked initial status"}, reply: "Started it. I’ll monitor it and keep this thread for coordination.", noise: []string{"ty"}},
		{user: "Aisha", text: "The agent sandbox must use proxy.usehivy.com with a ptok token. If a raw provider API key appears in sandbox env, that's a release blocker.", tools: []string{"read_file: checked agent runtime compile env", "bash: verified sandbox env var names"}, reply: "Verified. Sandbox env uses HIVY_PROXY_API_KEY and no provider key.", noise: []string{"important"}},
		...
		{user: "Maya", text: "Can you check why switching workspaces still shows the setup screen even though the org profile is ready?", tools: []string{"read_file: inspected active workspace state", "edit_file: refetched org profile status after workspace switch"}, reply: "Fixed. Workspace switching now refetches org profile state before rendering the flow.", noise: []string{"works locally"}},
		...
		{user: "Daniel", text: "Agent runtime changes need integration tests. Unit tests alone are not enough there.", tools: []string{"bash: ran go test ./internal/agentruntime", "edit_file: added compile integration coverage"}, reply: "Added integration coverage for agent runtime compile.", noise: []string{"ship it maybe"}},
		...
		{user: "Maya", text: "This repo picker is closer but the selected state still isn't obvious.", tools: []string{"read_file: inspected choice-card selected styling", "edit_file: aligned selected repository card state"}, reply: "Selected repos now use the same border and check treatment as setup cards.", noise: []string{"better"}},
		...
		{user: "Maya", text: "Vague feedback: the agents page feels off. I don't have better words yet.", tools: []string{"read_file: checked exploration page structure", "edit_file: tightened agents page spacing and primary action hierarchy"}, reply: "I made the smallest defensible layout improvement: clearer page title, tighter table, stronger primary action.", noise: []string{"yeah that's closer"}},
		...
		{user: "Daniel", text: "If a job is expected to take more than a few minutes, don't do it inline. Use a subagent and monitor it.", tools: []string{"subagent_task: started long-running docker runtime verification", "check_subagent_task_status: checked task status"}, reply: "Started and monitoring. This thread stays for decisions and status.", noise: []string{"ok"}},
...
func (s productionSession) toEvents(t *testing.T, orgID, agentID, sandboxID uuid.UUID, sessionID string, index int) []model.AgentSessionEvent {
	t.Helper()
	source := s.source
	if source == "" {
		source = "slack"
...
func distinctProductionSessionIDs(t *testing.T, db interface {
	...
	if err := db.Raw(
		"SELECT DISTINCT runtime_session_id FROM agent_session_events WHERE agent_id = ? AND sandbox_id = ? ORDER BY runtime_session_id",
		agentID, sandboxID,
