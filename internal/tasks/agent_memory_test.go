...
func TestBuildAgentRetainItem_BundlesSessionAtCheckpoint(t *testing.T) {
	orgID := uuid.New()
	agentID := uuid.New()
	sandboxID := uuid.New()
	...
	item, ok := buildAgentRetainItem(agent, AgentMemoryRetainPayload{
	...
	if !strings.Contains(item.Content, "rollback notes") || !strings.Contains(item.Content, "Agent Aria") {
...
func TestBuildAgentRetainItem_SkipsNoCheckpointAndSecrets(t *testing.T) {
	orgID := uuid.New()
	agentID := uuid.New()
	sandboxID := uuid.New()
	agent := &model.Agent{ID: agentID, OrgID: &orgID, Name: "Aria"}
	onlyUser := []model.AgentSessionEvent{
		memoryEvent(t, orgID, agentID, sandboxID, "S1", "user.message.received", map[string]any{"text": "remember this later"}),
	}
	if _, ok := buildAgentRetainItem(agent, AgentMemoryRetainPayload{AgentID: agentID, SandboxID: sandboxID, SessionID: "S1"}, onlyUser); ok {
	...
	if _, ok := buildAgentRetainItem(agent, AgentMemoryRetainPayload{AgentID: agentID, SandboxID: sandboxID, SessionID: "S1"}, withSecret); ok {
		t.Fatal("secret-looking transcript should not retain")
	}
}

func TestBuildAgentRetainItemWithReason_ReportsSkipReasons(t *testing.T) {
	orgID := uuid.New()
	agentID := uuid.New()
	sandboxID := uuid.New()
	agent := &model.Agent{ID: agentID, OrgID: &orgID, Name: "Aria"}
	...
	if _, ok, reason := buildAgentRetainItemWithReason(agent, AgentMemoryRetainPayload{AgentID: agentID, SandboxID: sandboxID, SessionID: "S1"}, onlyUser); ok || reason != "not_meaningful" {
		t.Fatalf("reason = %q ok=%t, want not_meaningful false", reason, ok)
	}
	withSecret := append(onlyUser, memoryEvent(t, orgID, agentID, sandboxID, "S1", "agent.message.sent", map[string]any{"text": "api_key=sk-secret"}))
	if _, ok, reason := buildAgentRetainItemWithReason(agent, AgentMemoryRetainPayload{AgentID: agentID, SandboxID: sandboxID, SessionID: "S1"}, withSecret); ok || reason != "secret_detected" {
		t.Fatalf("reason = %q ok=%t, want secret_detected false", reason, ok)
	}
}

func TestBuildAgentRetainItem_PreservesExplicitRememberFactsWithoutTools(t *testing.T) {
	orgID := uuid.New()
	agentID := uuid.New()
	sandboxID := uuid.New()
	agent := &model.Agent{ID: agentID, OrgID: &orgID, Name: "Aria"}
	...
	for _, want := range []string{"Nora owns invoice-failure alerts", "billing answers must use live data", "Agent Aria"} {
