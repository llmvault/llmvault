...
func TestStoreDelivery_IsIdempotentUnderRetry(t *testing.T) {
	...
	h := NewAgentTriggerStoreDeliveryHandler(db)
	payload := AgentTriggerStoreDeliveryPayload{
		OrgID:            orgID,
		AgentID:          agent.ID,
...
func newStoreDeliveryTask(t *testing.T, payload AgentTriggerStoreDeliveryPayload) *asynq.Task {
	t.Helper()
	encoded, err := json.Marshal(payload)
	if err != nil {
		t.Fatalf("marshal payload: %v", err)
	}
	return asynq.NewTask(TypeAgentTriggerStoreDelivery, encoded)
}
