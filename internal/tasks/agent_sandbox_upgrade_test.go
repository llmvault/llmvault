...
func TestAgentSandboxUpgradeWorker_SucceedsAndSchedulesOldRetirement(t *testing.T) {
	f := newAgentUpgradeFixture(t)
	if err := f.handler.Handle(context.Background(), agentUpgradeTask(t, f.upgrade.ID, f.agent.ID)); err != nil {
	...
	if upgrade.Status != model.AgentSandboxUpgradeStatusSucceeded || upgrade.Phase != model.AgentSandboxUpgradePhaseCompleted {
...
func TestAgentSandboxUpgradeWorker_CreateFailureLeavesOldRunning(t *testing.T) {
	f := newAgentUpgradeFixture(t)
	...
	if upgrade.Status != model.AgentSandboxUpgradeStatusFailed || upgrade.Phase != model.AgentSandboxUpgradePhaseCreatingNew {
		t.Fatalf("status/phase = %s/%s", upgrade.Status, upgrade.Phase)
	}
	var old model.Sandbox
	if err := f.db.First(&old, "id = ?", f.old.ID).Error; err != nil {
...
func TestAgentSandboxUpgradeWorker_BackupFailureLeavesOldRunning(t *testing.T) {
	f := newAgentUpgradeFixture(t)
	...
	if upgrade.Status != model.AgentSandboxUpgradeStatusFailed || upgrade.Phase != model.AgentSandboxUpgradePhaseBackup {
		t.Fatalf("status/phase = %s/%s", upgrade.Status, upgrade.Phase)
	}
	var old model.Sandbox
	if err := f.db.First(&old, "id = ?", f.old.ID).Error; err != nil {
...
func TestAgentSandboxUpgradeWorker_RestoreFailureRollsBackToOld(t *testing.T) {
	f := newAgentUpgradeFixture(t)
	...
	if upgrade.Status != model.AgentSandboxUpgradeStatusFailed || upgrade.Phase != model.AgentSandboxUpgradePhaseRestore {
		t.Fatalf("status/phase = %s/%s", upgrade.Status, upgrade.Phase)
	}
...
func TestAgentSandboxRetireHandler_ReleasesResourceAndArchivesOldSandboxAfterDelayTask(t *testing.T) {
	f := newAgentUpgradeFixture(t)
	if err := f.handler.Handle(context.Background(), agentUpgradeTask(t, f.upgrade.ID, f.agent.ID)); err != nil {
		t.Fatalf("handle upgrade: %v", err)
	}
	task := requireRetireTask(t, f.enqueuer)
	retireHandler := NewAgentSandboxRetireHandler(f.db, f.handler.orchestrator)
...
func TestAgentSandboxUpgradeWorker_SetsSnapshotIDFromConfig(t *testing.T) {
	f := newAgentUpgradeFixture(t)
	if err := f.handler.Handle(context.Background(), agentUpgradeTask(t, f.upgrade.ID, f.agent.ID)); err != nil {
		t.Fatalf("handle: %v", err)
	}
	var newSandbox model.Sandbox
	if err := f.db.First(&newSandbox, "id != ? AND agent_id = ?", f.old.ID, f.agent.ID).Error; err != nil {
...
func requireRetireTask(t *testing.T, c *enqueue.MockClient) *enqueue.EnqueuedTask {
	...
	for _, task := range c.Tasks() {
		if task.TypeName == TypeAgentSandboxRetire {
			return &task
		}
	}
