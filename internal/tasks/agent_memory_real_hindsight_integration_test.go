...
func TestRealHindsightAgentMemoryCheckpointFlow(t *testing.T) {
	...
	agent := model.Agent{ID: agentID, OrgID: &orgID, Name: "Aria", IsManaged: true, Status: "active"}
	if err := db.Create(&agent).Error; err != nil {
		t.Fatalf("create agent: %v", err)
	}
	if err := db.Create(&model.Sandbox{ID: sandboxID, OrgID: &orgID, AgentID: &agentID, ExternalID: "real-hindsight-sb", RuntimeURL: "http://runtime", EncryptedRuntimeSecret: []byte("x"), Status: "running"}).Error; err != nil {
		t.Fatalf("create sandbox: %v", err)
	}
	createMemorySession(t, db, orgID, agentID, sandboxID, sessionID)

	seededEvents := []model.AgentSessionEvent{
	...
	itemPreview, ok := buildAgentRetainItem(&agent, AgentMemoryRetainPayload{
	...
	handler := NewAgentMemoryRetainHandler(db, client, nil)
	task, _, err := NewAgentMemoryRetainTask(AgentMemoryRetainPayload{
		AgentID:     agentID,
		SandboxID:   sandboxID,
		SessionID:   sessionID,
	...
	if err := db.Model(&model.AgentSessionEvent{}).
		Where("agent_id = ? AND sandbox_id = ? AND runtime_session_id = ? AND retained_at IS NOT NULL", agentID, sandboxID, sessionID).
	...
	if itemPreview.DocumentID != "agent-session:"+sandboxID.String()+":"+sessionID {
		t.Fatalf("preview document id = %q", itemPreview.DocumentID)
	}
}

func TestRealHindsightAgentMemoryProductionWorkload(t *testing.T) {
	baseURL := os.Getenv("HIVY_HINDSIGHT_API_URL")
	if baseURL == "" {
		baseURL = "http://localhost:8888"
	}
	...
	if err := db.Create(&model.Sandbox{ID: sandboxID, OrgID: &orgID, AgentID: &agentID, ExternalID: "production-workload-sb", RuntimeURL: "http://runtime", EncryptedRuntimeSecret: []byte("x"), Status: "running"}).Error; err != nil {
		t.Fatalf("create sandbox: %v", err)
	}
	...
	for i, session := range sessions {
		...
		if _, ok := buildAgentRetainItem(&agent, AgentMemoryRetainPayload{
			AgentID: agentID, SandboxID: sandboxID, SessionID: sessionID, SourceEvent: "agent.message.sent",
		}, events); ok {
