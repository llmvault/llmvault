...
func TestAgentTriggerDispatch_RetryDeliversOnlyFromFailedTrigger(t *testing.T) {
	...
	agent := model.Agent{ID: uuid.New(), OrgID: &orgID, Name: "Aria", Model: "test-model", Status: "active", IsManaged: true}
	...
	t.Cleanup(func() {
		db.Where("org_id = ?", orgID).Delete(&model.AgentTriggerDelivery{})
		db.Where("org_id = ?", orgID).Delete(&model.AgentTrigger{})
		db.Where("org_id = ?", orgID).Delete(&model.AgentSession{})
	...
	orch := sandbox.NewOrchestrator(db, &agentUpgradeProvider{endpoint: server.URL}, encKey, cfg)
	...
	dispatchPayload := AgentTriggerDispatchPayload{
		OrgID:        orgID,
		ConnectionID: connID,
		Provider:     "github",
		EventType:    "issues",
	...
	task, _, err := NewAgentTriggerDispatchTask(dispatchPayload)
	if err != nil {
