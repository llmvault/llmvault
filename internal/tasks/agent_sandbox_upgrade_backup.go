...
type agentSandboxBackupMetadata struct {
	Key    string `json:"key"`
	SHA256 string `json:"sha256"`
	Bytes  int64  `json:"bytes"`
}

func (h *AgentSandboxUpgradeHandler) runBackup(ctx context.Context, upgrade *model.AgentSandboxUpgrade, agent *model.Agent, sb *model.Sandbox) (*agentSandboxBackupMetadata, error) {
	key := agentSandboxUpgradeBackupKey(upgrade.OrgID, agent.ID, upgrade.ID)
	uploadURL, err := h.store.PresignedPutURL(ctx, key, time.Hour)
	if err != nil {
		return nil, fmt.Errorf("presign backup upload: %w", err)
	}
	cmd := buildAgentSandboxBackupCommand(uploadURL)
	output, err := h.orchestrator.ExecuteCommandWithTimeout(ctx, sb, cmd, agentSandboxUpgradeCommandTimeout)
	if err != nil {
		return nil, fmt.Errorf("backup command failed: %w", err)
	}
	var meta agentSandboxBackupMetadata
...
func (h *AgentSandboxUpgradeHandler) verifyAndRecordBackup(ctx context.Context, upgrade *model.AgentSandboxUpgrade, meta *agentSandboxBackupMetadata) error {
...
func (h *AgentSandboxUpgradeHandler) runRestore(ctx context.Context, meta *agentSandboxBackupMetadata, sb *model.Sandbox) error {
	url, err := h.store.PresignedURL(ctx, meta.Key, time.Hour)
	if err != nil {
		return fmt.Errorf("presign backup: %w", err)
	}
	cmd := buildAgentSandboxRestoreCommand(url, meta.SHA256)
	output, err := h.orchestrator.ExecuteCommandWithTimeout(ctx, sb, cmd, agentSandboxUpgradeCommandTimeout)
	if err != nil {
...
func agentSandboxUpgradeBackupKey(orgID, agentID, upgradeID uuid.UUID) string {
