...
func TestTriggerDispatchTaskOptionsAreReturnedSeparately(t *testing.T) {
	task, opts, err := NewAgentTriggerDispatchTask(AgentTriggerDispatchPayload{
		OrgID:      uuid.New(),
	...
	if task.Type() != TypeAgentTriggerDispatch {
...
func TestTriggerDispatchTaskBuilderReturnsOptions(t *testing.T) {
	builder, ok := lookupTaskBuilder(TypeAgentTriggerDispatch)
	if !ok {
		t.Fatalf("no task builder registered for %s", TypeAgentTriggerDispatch)
	}
	payload, err := json.Marshal(AgentTriggerDispatchPayload{OrgID: uuid.New(), DeliveryID: "d2"})
	if err != nil {
