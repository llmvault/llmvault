...
func memoryEvent(t *testing.T, orgID, agentID, sandboxID uuid.UUID, sessionID, eventType string, payload map[string]any) model.AgentSessionEvent {
	t.Helper()
	...
	return model.AgentSessionEvent{
		...
		AgentID:        agentID,
		SandboxID:      sandboxID,
		AgentSessionID: memorySessionID(orgID, agentID, sandboxID, sessionID),
		SessionID:      sessionID,
		EventType:      eventType,
		Source:         "slack",
