...
func TestAgentProxyTokenRefreshHandler_InjectsNewTokenRevokesOldAndSchedulesNext(t *testing.T) {
	...
	if err := f.db.Model(&model.Token{}).Where("jti = ?", oldToken.JTI).
		Update("created_at", time.Now().Add(-2*agentProxyTokenRevokeGrace)).Error; err != nil {
		t.Fatalf("backdate old token: %v", err)
	}

	task, _, err := NewAgentProxyTokenRefreshTask(AgentProxyTokenRefreshPayload{
		AgentID:     f.agent.ID,
		SandboxID:   f.sandbox.ID,
		ScheduledAt: oldToken.ExpiresAt.Add(-agentProxyTokenRefreshLead),
	})
	if err != nil {
	...
	runtimeSecret := f.runtime.lastEnv[agentruntime.AgentEnvRuntimeSecret]
	agentID := f.runtime.lastEnv[agentruntime.AgentEnvAgentID]
	f.runtime.mu.Unlock()
	if envCalls != 1 {
	...
	if agentID != f.agent.ID.String() {
		t.Fatalf("runtime config agent id env = %q, want %s", agentID, f.agent.ID)
	}
	...
	if activeCount != 1 {
		t.Fatalf("active agent proxy tokens = %d, want 1", activeCount)
	}
...
func TestAgentProxyTokenRefreshHandler_DoesNotRevokeConcurrentlyMintedToken(t *testing.T) {
	f := newAgentProxyTokenRefreshFixture(t, 0)
...
func TestAgentProxyTokenRefreshHandler_RevokesMintedTokenWhenRuntimeRejectsEnv(t *testing.T) {
	f := newAgentProxyTokenRefreshFixture(t, http.StatusInternalServerError)
...
func TestNextAgentProxyTokenRefreshAt_UsesActiveStartupTokenExpiry(t *testing.T) {
	f := newAgentProxyTokenRefreshFixture(t, 0)
	...
	now := startupToken.ExpiresAt.Add(-12 * agentProxyTokenRefreshLead)

	got, err := nextAgentProxyTokenRefreshAt(context.Background(), f.db, &f.agent, f.sandbox.ID, now)
	if err != nil {
		t.Fatalf("next refresh: %v", err)
	}
	want := startupToken.ExpiresAt.Add(-agentProxyTokenRefreshLead).UTC().Truncate(time.Microsecond)
	if !got.Equal(want) {
