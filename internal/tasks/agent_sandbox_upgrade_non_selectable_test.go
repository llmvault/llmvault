...
func TestAgentSandboxUpgradeKeepsNewSandboxNonSelectableUntilSync(t *testing.T) {
	f := newAgentUpgradeFixture(t)

	selector := agentRuntimeSelector(f.db, f.handler.compileDeps)
...
func TestAgentSandboxUpgradeRollbackRunsOnCancelledContext(t *testing.T) {
	f := newAgentUpgradeFixture(t)
	...
	_ = f.handler.Handle(ctx, agentUpgradeTask(t, f.upgrade.ID, f.agent.ID))

	var upgrade model.AgentSandboxUpgrade
	if err := f.db.First(&upgrade, "id = ?", f.upgrade.ID).Error; err != nil {
		t.Fatalf("load upgrade: %v", err)
	}
	if upgrade.Status != model.AgentSandboxUpgradeStatusFailed {
