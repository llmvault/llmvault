...
func (h *AgentTriggerDispatchHandler) fetchGitHubMentionComments(ctx context.Context, payload AgentTriggerDispatchPayload, event githubMentionEvent) []githubMentionComment {
	...
	if err != nil {
		logging.FromContext(ctx).WarnContext(ctx, "github mention: fetch comments failed",
			"repository", event.Repo, "number", event.Number, "error", err)
		return nil
	}
	items := proxyResponseArray(resp)
	comments := make([]githubMentionComment, 0, githubMentionMaxComments)
	for _, item := range items {
		...
		if body == event.Body && author == event.MentionedBy {
			continue
		}
		comments = append(comments, githubMentionComment{author: author, body: body})
	}
