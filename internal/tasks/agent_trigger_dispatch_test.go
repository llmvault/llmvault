...
func TestAgentTriggerCompileMessage_UsesCatalogRefsAndOmitsRawPayload(t *testing.T) {
	triggerID := uuid.New()
	...
	compiled := (&AgentTriggerDispatchHandler{catalog: catalog.Global()}).
...
func TestAgentTriggerCompileMessage_HTTPIncludesSubmittedBody(t *testing.T) {
	trigger := model.AgentTrigger{
		ID:           uuid.New(),
		TriggerType:  "http",
	...
	compiled := (&AgentTriggerDispatchHandler{catalog: catalog.Global()}).
		compileMessage(AgentTriggerDispatchPayload{DeliveryID: "http-1"}, trigger, raw)
...
func TestAgentTriggerConversationCreationEnqueuesMemoryRetain(t *testing.T) {
	...
	if payload.AgentSessionID != conv.ID || payload.AgentID != agent.ID || payload.SandboxID != sb.ID {
...
func TestAgentTriggerDispatchSyncRuntime_PushesFullRuntimeConfig(t *testing.T) {
	db := openTasksMemoryTestDB(t)
