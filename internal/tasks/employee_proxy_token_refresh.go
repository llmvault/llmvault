...
func NewEmployeeProxyTokenRefreshTask(payload EmployeeProxyTokenRefreshPayload) (*asynq.Task, []asynq.Option, error) {
	...
	opts := []asynq.Option{
		...
		asynq.TaskID(EmployeeProxyTokenRefreshTaskID(payload.SandboxID, payload.ScheduledAt)),
	}
...
func EmployeeProxyTokenRefreshTaskID(sandboxID uuid.UUID, scheduledAt time.Time) string {
	return fmt.Sprintf("employee-proxy-token-refresh:%s:%d", sandboxID, scheduledAt.UTC().Unix())
}
