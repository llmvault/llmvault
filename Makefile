...
.PHONY: desktop-dev desktop-build desktop-test desktop-e2e
...
microsandbox-build:
	go build -ldflags="-s -w -X main.version=$(VERSION) -X main.commit=$(COMMIT)" \
		-o $(MICROSANDBOX_BINARY) ./cmd/microsandbox
...
microsandbox-release-darwin-arm64:
	GOOS=darwin GOARCH=arm64 go build -ldflags="-s -w -X main.version=$(VERSION) -X main.commit=$(COMMIT)" \
		-o dist/microsandbox-darwin-arm64 ./cmd/microsandbox
...
# Run the Apps REALTIME flagship E2E: the harness itself seeds a sheet, builds
# the app template directly (no agent, no LLM), publishes it through the REST
# version endpoint, and asserts the live SSE pipe end to end — refresh on
# subscribe, insert/update/delete rows_changed frames with snapshots,
# cross-sheet isolation, concurrent-client fan-out, and refetch agreement.
# Requires the compose stack plus the app sandbox image (and a host with
# go/node/npm to build the template).
...
test-real-nango: ensure-nango-image
	@secret=$$($(DEV_DOCKER_COMPOSE) exec -T postgres sh -lc 'psql -U "$$POSTGRES_USER" -d nango -Atc "SELECT secret_key FROM nango._nango_environments WHERE name='\''prod'\'' LIMIT 1"'); \
...
ci-quality:
	...
	./scripts/check-bare-goroutines.sh
	./scripts/check-migrations.sh
	./scripts/check-deployment-workflows.sh

check-deployment-workflows:
	./scripts/check-deployment-workflows.sh

ensure-nango-image:
	@HIVY_NANGO_COMPOSE_ENV_FILE="$(NANGO_COMPOSE_ENV_FILE)" ./scripts/ensure-nango-image.sh

# Start local Nango first and print the generated Hivy API secret.
dev-nango: ensure-nango-image
	$(DEV_DOCKER_COMPOSE) up -d postgres redis nango
	@echo "Waiting for Nango..."
	@until $(DEV_DOCKER_COMPOSE) exec -T postgres sh -lc 'pg_isready -U "$$POSTGRES_USER" -q' 2>/dev/null; do sleep 1; done
	@until curl -fsS http://localhost:$${HIVY_COMPOSE_NANGO_PORT:-23003}/health >/dev/null 2>&1; do sleep 1; done
	@secret=$$($(DEV_DOCKER_COMPOSE) exec -T postgres sh -lc 'psql -U "$$POSTGRES_USER" -d nango -Atc "$(NANGO_SECRET_SQL)"'); \
...
dev-nango-secret: ensure-nango-image
	@secret=$$($(DEV_DOCKER_COMPOSE) exec -T postgres sh -lc 'psql -U "$$POSTGRES_USER" -d nango -Atc "$(NANGO_SECRET_SQL)"'); \
	if [ -z "$$secret" ]; then echo "Nango secret not found; run make dev-nango first" >&2; exit 1; fi; \
	printf '%s\n' "$$secret"

dev-migrate: ensure-nango-image
	@set -a; [ ! -f .env ] || . ./.env; set +a; \
	secret=$$($(MAKE) -s dev-nango-secret); \
	port=$$($(DEV_DOCKER_COMPOSE) port postgres 5432 | awk -F: '{print $$NF}' | tail -1); \
...
infra-up: ensure-nango-image
	@$(MAKE) -s dev-nango >/dev/null
	$(DEV_DOCKER_COMPOSE) up -d qdrant minio minio-setup

app-up: ensure-nango-image
	@secret=$$($(MAKE) -s dev-nango-secret); \
	echo "Starting Hivy app containers with local Nango secret"; \
	HIVY_NANGO_SECRET_KEY="$$secret" $(DEV_DOCKER_COMPOSE) up -d --no-deps $(DEV_APP_SERVICES)

app-up-build: ensure-nango-image
	@secret=$$($(MAKE) -s dev-nango-secret); \
...
dev: infra-up
	...
	HIVY_NANGO_SECRET_KEY="$$secret" $(DEV_DOCKER_COMPOSE) up --no-deps $(DEV_APP_SERVICES)

dev-build: infra-up
	@$(MAKE) -s dev-migrate
	@secret=$$($(MAKE) -s dev-nango-secret); \
	echo "Building and starting Hivy dev stack with local Nango secret"; \
	HIVY_NANGO_SECRET_KEY="$$secret" $(DEV_DOCKER_COMPOSE) up --build --no-deps $(DEV_APP_SERVICES)
...
desktop-test:
	...
	pnpm --dir apps/web exec vitest run lib/desktop/bridge.test.ts 'app/w/(chat)/_lib/session-sandbox-access.test.ts' 'app/w/(chat)/_lib/runtime-repos.test.ts' 'app/w/(chat)/_stores/session-stream-manager.test.ts'
	go test ./internal/handler -run '^(TestNormalizeDesktopRuntimeConfig|TestSessionUsage|TestSessionEventsPaginationUsesSequenceWhenBatchTimestampsMatch|TestSessionTranscriptEventsCompactRuntimeDeltasBeforePagination)$$' -count=1
	go test ./internal/tasks -run '^(TestSandboxAutoSleepQueriesExcludeDesktopRuntimes|TestSyncSandboxTurnUsageSkipsDesktopProvider)$$' -count=1
	cargo test --manifest-path sandboxes/runtime/Cargo.toml -p storage volatile --locked

# Requires make dev (or make up) and a running desktop-dev process using the
# same HIVY_DESKTOP_RUNTIME_SECRET value.
desktop-e2e:
	apps/desktop/scripts/e2e-local.sh
