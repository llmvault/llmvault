...

{
  ...
  missing=0
  for key in HIVY_RUNTIME_SECRET HIVY_PROXY_API_KEY HIVY_AGENT_MODEL HIVY_AGENT_BASE_URL HIVY_AGENT_API_KEY_ENV HIVY_EMPLOYEE_ID HIVY_CONTROL_PLANE_URL HIVY_DRIVE_UPLOAD_BEARER HIVY_WORKSPACE_ROOT HIVY_DB_PATH HIVY_RUNTIME_BIND_ADDR HIVY_SANDBOX_ID HIVY_ORG_ID HIVY_DRIVE_UPLOAD_URL; do
    eval "value=\${$key:-}"
    if [ -z "$value" ]; then
      echo "- $key"
      missing=1
