// agent-debug-pack uploads and runs an agent sandbox debug collector,
// downloads the resulting archive, extracts it under /tmp, and prints the
// absolute path to every extracted file.
//
// Usage:
//
//	go run ./cmd/agent-debug-pack -id <daytona-sandbox-id>
package main
...
func main() {
	...
	remoteScript := fmt.Sprintf("/tmp/hivy-agent-debug-pack-%s.sh", stamp)
...
func uploadFile(ctx context.Context, client *toolbox.APIClient, remotePath string, data []byte) error {
	tmp, err := os.CreateTemp("", "agent-debug-script-*")
	if err != nil {
		return err
	}
