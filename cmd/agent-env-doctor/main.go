// agent-env-doctor prints a redacted agent sandbox env report.
//
// Usage:
//
//	go run ./cmd/agent-env-doctor -id <daytona-sandbox-id>
package main
...
type doctorOutput struct {
	...
	PID       string                             `json:"pid"`
	Entries   []agentruntime.AgentEnvReportEntry `json:"entries"`
}

func main() {
	sandboxID := flag.String("id", "", "Daytona sandbox ID")
	pid := flag.String("pid", "", "Specific process PID inside the sandbox; defaults to agent runtime process or PID 1")
	jsonOut := flag.Bool("json", true, "Emit JSON instead of a table")
	includeUnexpected := flag.Bool("include-unexpected", false, "Include env keys not listed in the agent env catalog")
	...
	result, err := executeToolboxCommand(ctx, os.Getenv("HIVY_DAYTONA_API_URL"), os.Getenv("HIVY_DAYTONA_API_KEY"), *sandboxID, agentEnvKeysCommand(*pid))
	if err != nil {
	...
	output := doctorOutput{
		SandboxID: *sandboxID,
		PID:       payload.PID,
		Entries:   agentruntime.AgentEnvReportFromEnv(payload.Env, *includeUnexpected, *includeSensitive),
	}
	if *jsonOut {
...
func agentEnvKeysCommand(pid string) string {
	prefix := ""
	if pid != "" {
		prefix = "AGENT_ENV_PID=" + shellQuote(pid) + " "
	}
