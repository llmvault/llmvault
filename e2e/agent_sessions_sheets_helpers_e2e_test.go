...
func sheetsE2EConnectMCP(t *testing.T, ctx context.Context, db *gorm.DB, svc *sheets.Service, orgID, agentID uuid.UUID) *mcp.ClientSession {
	...
	server, err := mcpserver.BuildServer(ctx, token, db, nil, nil, nil, nil, nil, skills.NewToolsFunc(db, "http://localhost:3000"), nil, sheets.NewToolsFunc(svc), nil, nil, nil) //nolint:contextcheck // tool handlers receive their own request context from the MCP server at call time.
	if err != nil {
