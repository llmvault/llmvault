...
func TestAgentRuntimeCodingTaskE2E(t *testing.T) {
	...
	assertRuntimeParentSubagentStream(t, trace, "bearer parent stream", events)
	assertRuntimeParentSubagentStream(t, trace, "direct browser stream", directSessionEvents)
	assertRuntimeE2EEvents(t, trace, events)
	requiredFinalText := append([]string{"E2E_PASS", agentRuntimeE2EToken, "REAL_REPOS_CONFIRMED", "BASH_SEARCH_CONFIRMED", "APPLY_PATCH_CONFIRMED", "LSP_CONFIRMED", "ALL_LSP_SERVERS_CONFIRMED"}, agentRuntimeE2EAllSubagentMarkers()...)
...
func agentRuntimeCodingRequest() map[string]any {
	...
	return map[string]any{
		...
		"text": strings.Join([]string{
			...
			"4. Use bash with fd to find serializer.py under repos/itsdangerous: fd --type f '^serializer\\.py$' repos/itsdangerous.",
			"5. Use bash with fd to list Go files under repos/gjson: fd --type f --extension go . repos/gjson.",
			"6. Use bash with rg to search for func Get in Go files under repos/gjson: rg -n --glob '*.go' 'func Get' repos/gjson.",
			"7. Use bash with rg and repeated -e flags to search repos for func Get or class Serializer: rg -n -e 'func Get' -e 'class Serializer' --glob '*.{go,py}' repos.",
			...
			"12. Use apply_patch to add TOOLING_E2E.md containing the retrieved token and the phrases REAL_REPOS_CONFIRMED, BASH_SEARCH_CONFIRMED, APPLY_PATCH_CONFIRMED, LSP_CONFIRMED, and ALL_LSP_SERVERS_CONFIRMED. The patch argument must follow this exact shape: *** Begin Patch\\n*** Add File: TOOLING_E2E.md\\n+token: <retrieved token>\\n+REAL_REPOS_CONFIRMED\\n+BASH_SEARCH_CONFIRMED\\n+APPLY_PATCH_CONFIRMED\\n+LSP_CONFIRMED\\n+ALL_LSP_SERVERS_CONFIRMED\\n*** End Patch",
			...
			"15. Dispatch the three configured Hakaree subagents in parallel. In one assistant tool-call batch, call subagent_task three times and use the returned completed tool results directly. Use agent codebase-explorer with goal: FLAGSHIP_E2E_TEST_SESSION: do exactly 2-3 cheap local tool calls, specifically read_file calc.py and bash with fd -e py . plus rg -n --glob '*.py' runtime_token ., then final exactly " + codebaseMarker + ". Use agent librarian with goal: FLAGSHIP_E2E_TEST_SESSION: do exactly 2-3 cheap local tool calls, specifically read_file test_calc.py and bash with fd '^calc\\.py$' . plus fd --type f --extension json . lsp-fixtures, then final exactly " + librarianMarker + ". Use agent oracle with goal: FLAGSHIP_E2E_TEST_SESSION: do exactly 2-3 cheap local tool calls, specifically read_file calc.py, bash with rg -n -e runtime_token -e helper_phrase --glob '*.py' ., and lsp diagnostics filePath calc.py, then final exactly " + oracleMarker + ".",
			"16. Do not call any subagent status tool. Each subagent_task result must contain its marker before you continue.",
			"17. Use bash to run python3 -m unittest -v.",
			"18. After the tests pass and all three subagent_task results are available, final answer must include E2E_PASS, the exact token, REAL_REPOS_CONFIRMED, BASH_SEARCH_CONFIRMED, APPLY_PATCH_CONFIRMED, LSP_CONFIRMED, ALL_LSP_SERVERS_CONFIRMED, and " + codebaseMarker + ", " + librarianMarker + ", " + oracleMarker + ".",
		}, "\n"),
