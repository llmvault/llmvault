...
func newHarness(t *testing.T) *testHarness {
	...
	proxyHandler := handler.NewProxyHandler(db, nil, cm, nil, proxy.NewTransport())
...
func (h *testHarness) storeCredential(t *testing.T, org model.Org, baseURL, authScheme, apiKey string) model.Credential {
	...
	body := fmt.Sprintf(`{"label":"e2e-test","provider_id":"atlascloud","base_url":%q,"auth_scheme":%q,"api_key":%q}`,
