...
**`attachment`** — cell value = array of S3 object keys under a new `AssetPolicy` type `sheet_attachment` (prefix `pub/o/{orgID}/sheets/attachments/`, ≤25 MB/file). Uploads go through the existing sign→PUT flow with `sign(sheet_attachment)`. Decision (2026-07-02, superseded same day): the first cut shared one pure-string predicate (`sheets.ValidAttachmentKey`, org-prefix only) between cell acceptance and download-url and rejected agent drive keys — but that broke the documented agent flow, since sandbox agents can only upload via the drive endpoint (`pub/e/{agentID}/…`) and cannot reach the org-session-authed `/v1` sign/upload endpoints. Superseding decision (2026-07-02): the shared admission rule is `sheets.Service.AuthorizeObjectKeys` (applied uniformly to attachment cells, CSV import object keys, and download-url): any org-owned key under `pub/o/{orgID}/` with no `..` traversal is accepted string-only (`ValidAttachmentKey` remains that fast path), AND agent drive keys `pub/e/{agentID}/…` are accepted when the agent ID parses as a UUID and one batched indexed lookup proves the agent belongs to the caller's org (foreign-org or non-existent agents still rejected; malformed keys rejected before any DB hit; the check runs only on write/download-url paths, never per-row reads). Downloads via presigned GET URLs. Lifecycle v1: objects kept until row hard-delete (org cascade) — no reaping on archive. No query-compiler involvement beyond `is_empty`/`is_not_empty`.
