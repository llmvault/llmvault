...
export function FilesView() {
  ...
  const assets = React.useMemo<ApiAsset[]>(
    () => query.data?.data ?? [],
    [query.data]
  )

  const entries = React.useMemo<Entry[]>(
    () => buildEntries(assets, folder),
    [assets, folder]
  )
  ...
  return (
    <>
      <ScrollArea className="h-full">
        <div className="flex flex-col px-3 pt-2 pb-28">
          <Breadcrumb folder={folder} onNavigate={setFolder} />

          <div className="grid grid-cols-[1fr_120px_140px] gap-3 px-3 pb-2 font-mono text-[10px] tracking-[1.2px] text-muted-foreground/50 uppercase">
            <span>Name</span>
            <span>Size</span>
            <span>Modified</span>
          </div>
          ...
          {isEmpty ? (
            <EmptyState text="No files have been uploaded to this drive yet." />
          ) : null}
          ...
          <div className="flex flex-col gap-0.5">
            {entries.map((entry, i) => (
              <Row
                key={
                  entry.kind === "folder" ? `f:${entry.name}` : entry.asset.id
                }
                entry={entry}
...
function buildEntries(assets: ApiAsset[], folder: string): Entry[] {
  ...
  files.sort((a, b) =>
    (b.asset.created_at ?? "").localeCompare(a.asset.created_at ?? "")
  )
  ...
  if (entry.kind === "folder") {
    return (
      <button
        ...
        style={style}
        className="grid animate-in grid-cols-[1fr_120px_140px] items-center gap-3 rounded-lg px-3 py-2 text-left transition-colors duration-150 fade-in slide-in-from-bottom-1 hover:bg-muted/50"
      >
...
function KindBadge({ kind }: { kind: FileKind | "folder" }) {
  const config: Record<
    FileKind | "folder",
    { icon: typeof BrowserIcon; className: string }
  > = {
