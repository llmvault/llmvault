...
export function PdfPreview({ asset }: { asset: Asset }) {
  ...
  return (
    <div className="flex h-full w-full flex-col items-center justify-center gap-5 px-6 pt-24 pb-24">
      <div
        ref={containerRef}
      ...
      {numPages && numPages > 1 ? (
        <div className="flex items-center gap-2 rounded-full bg-card px-2 py-1.5 shadow-sm ring-1 ring-border">
          <button
            type="button"
            onClick={goPrev}
            disabled={page === 1}
          ...
          <span className="px-2 font-mono text-[11px] tracking-[0.04em] text-foreground tabular-nums">
            {page} <span className="text-muted-foreground/50">/</span>{" "}
            {numPages}
          </span>
          <button
            type="button"
...
function DocumentLoading() {
  return (
    <div className="flex h-72 w-[480px] animate-pulse items-center justify-center rounded-md bg-muted font-mono text-[10px] tracking-[0.16em] text-muted-foreground uppercase">
      Rendering page
    </div>
  )
}
