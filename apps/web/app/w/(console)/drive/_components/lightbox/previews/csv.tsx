...
export function CsvPreview({ asset }: { asset: Asset }) {
  ...
  return (
    <div className="flex h-full w-full items-start justify-center overflow-hidden px-6 pt-24 pb-24">
      <div className="flex w-full max-w-[1100px] flex-col gap-3">
        ...
        <div className="overflow-auto rounded-md bg-card ring-1 ring-border">
          ...
          {state.status === "error" ? (
            <TableError message={state.message} />
          ) : null}
          {state.status === "ready" ? (
...
function Meta({ state }: { state: ParseState }) {
  ...
  return (
    <div className="flex items-center gap-3 px-1 font-mono text-[10.5px] tracking-[0.14em] text-muted-foreground uppercase">
      <span>
  ...
  return (
    <table className="min-w-full border-collapse text-[12px]">
      <thead className="sticky top-0 z-10 bg-card supports-backdrop-filter:bg-card/85 supports-backdrop-filter:backdrop-blur-md">
        <tr>
          <th className="border-b border-border py-2 pr-3 pl-4 text-left font-mono text-[10px] font-normal tracking-[0.14em] text-muted-foreground uppercase">
            #
          </th>
          {headers.map((h, i) => (
            <th
              key={i}
              className="border-b border-border px-3 py-2 text-left font-mono text-[10px] font-normal tracking-[0.14em] text-foreground uppercase"
            >
              {h || `col_${i + 1}`}
            </th>
          ))}
      ...
      <tbody>
        {rows.map((row, ri) => (
          ...
          >
            <td className="py-1.5 pr-3 pl-4 font-mono text-[10.5px] text-muted-foreground/70 tabular-nums">
              {ri + 1}
            </td>
            {row.map((cell, ci) => (
