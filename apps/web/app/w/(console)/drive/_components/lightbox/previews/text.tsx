    ...
    fetch(asset.publicUrl)
      .then(async (res) => {
        ...
        if (blob.size > MAX_BYTES) {
          throw new Error(
            "File is too large to render inline. Try downloading it."
          )
        }
        return blob.text()
      })
  ...
  return (
    <div className="flex h-full w-full items-start justify-center overflow-hidden px-6 pt-24 pb-24">
      <div className="flex w-full max-w-[820px] overflow-hidden rounded-md bg-card ring-1 ring-border">
        ...
        {state.status === "ready" ? (
          <pre className="m-0 max-h-[calc(100vh-12rem)] w-full overflow-auto px-8 py-7 font-mono text-[12px] leading-[1.7] break-words whitespace-pre-wrap text-foreground">
            {state.text}
          </pre>
        ) : null}
      </div>
