...
export function SessionPlanCard({
  ...
  const content = (
    <div
      className={
        inline ? "relative w-full" : "relative mx-auto w-full max-w-3xl px-4"
      }
    >
      <AnimatePresence initial={false}>
        {expanded ? (
          <motion.div
            ...
            className={`pointer-events-auto absolute bottom-[calc(100%+0.5rem)] max-h-[min(26rem,calc(100vh-12rem))] overflow-y-auto rounded-2xl border border-border bg-surface px-4 py-3 text-surface-foreground shadow-lg ${
              inline ? "inset-x-0" : "inset-x-4"
            }`}
          >
      ...
      <motion.button
        ...
        transition={popoverTransition}
        className="focus-visible:ring-ring pointer-events-auto flex min-h-12 w-full items-center gap-3 rounded-2xl border border-border bg-surface px-4 py-3 text-left text-sm text-surface-foreground shadow-sm transition-colors hover:bg-surface-secondary focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-background focus-visible:outline-none"
      >
