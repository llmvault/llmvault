...
const VideoPreview = dynamic(
  () => import("./previews/video").then((m) => m.VideoPreview),
  {
    ssr: false,
...
const AudioPreview = dynamic(
  () => import("./previews/audio").then((m) => m.AudioPreview),
  {
    ssr: false,
    loading: PreviewSkeleton,
  }
...
const PdfPreview = dynamic(
  () => import("./previews/pdf").then((m) => m.PdfPreview),
  {
    ssr: false,
    loading: PreviewSkeleton,
  }
...
const CsvPreview = dynamic(
  () => import("./previews/csv").then((m) => m.CsvPreview),
  {
    ssr: false,
    loading: PreviewSkeleton,
  }
...
const CodePreview = dynamic(
  () => import("./previews/code").then((m) => m.CodePreview),
  {
    ssr: false,
    loading: PreviewSkeleton,
  }
...
function inferKind(a: Asset): AssetKind {
  ...
  if (
    ...
    return "image"
  if (ct.startsWith("video/") || ["mp4", "mov", "webm", "mkv"].includes(ext))
    return "video"
  if (
    ct.startsWith("audio/") ||
  ...
  if (
    ct.startsWith("text/") ||
    ["txt", "md", "log", "yml", "yaml", "ini", "toml"].includes(ext)
  )
    return "text"
  return "unknown"
}
