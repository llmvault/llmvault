...
GitHub Actions does not build, publish, promote, or deploy Hivy container
images. Pushes to `main` run validation only. Publishing a GitHub Release creates
release metadata only and does not start an image or deployment workflow.

The retired automation previously published application, Microsandbox, sandbox
runtime, sandbox app, and Daytona-targeted images to GHCR. It also published
runtime manifests, created Daytona snapshots, and patched staging or production
Deployments. Those workflow entry points have been removed.

Kustomize and an administrator's `kubectl` apply now manage live workloads.
There is no in-cluster GitOps controller. Operators must choose an existing
immutable image digest, update the intended overlay, validate it, and apply it
explicitly.

## Private Kubernetes API access

The Kubernetes API is not exposed as a general public endpoint. The deployment
helper can try the addresses in `K8S_TUNNEL_HOSTS` and forward a local port to
...
Each namespace may contain a legacy `github-actions-deployer` ServiceAccount.
Its Role can only `get` and `patch` the named `backend-api`, `backend-worker`,
and `web` Deployments in that namespace. No active workflow uses this account.
It cannot list Deployments, inspect Pods, read Secrets or ConfigMaps, touch data
services, or modify cluster-wide objects.

The retired deployment workflow used these entries in both GitHub Environments:
...
## What the deployment helper changes
...
configuration, or deploy Microsandbox components. No workflow invokes it. The
image digests in each `kustomization.yaml` are static records and are not
rewritten by CI. Before an intentional apply, update the overlay digests to the
version that should remain live.
