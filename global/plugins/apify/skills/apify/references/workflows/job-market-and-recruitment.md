...
### Gotcha
No dedicated Upwork Actor exists in Apify Store - verify by searching the Store (`GET $APIFY_API/store?search=upwork`) for community options before defaulting to `apify/playwright-scraper`. Upwork pages are JS-heavy so Playwright is required over basic HTTP scraping. For high-frequency monitoring (every 15 min), store seen job URLs to avoid re-processing duplicates.
