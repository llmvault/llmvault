...
### Gotcha
No market-specific Actor exists for most construction portals (e.g., 99acres). Search the Store (`GET $APIFY_API/store?search=real%20estate`) to check for community-built options before using `apify/playwright-scraper`. For JS-heavy portals, `playwright-scraper` is required. Step 2 cleans raw output into structured fields - pipe all Step 1 URLs through it.
