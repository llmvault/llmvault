...
### Gotcha
Amazon aggressively rotates prices and sometimes shows regional prices. Always store `currency` alongside `price`. For review text (not just counts), search the Store (`GET $APIFY_API/store?search=amazon%20reviews`) for a dedicated Actor.
