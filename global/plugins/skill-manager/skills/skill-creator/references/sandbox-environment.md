...
- Only registered preview ports (defaults include 3000, 5173, 8000, 8080; org-configurable) are reachable from outside, at `https://{port}-{sandbox_id}.preview.usehivy.com`. A server on an unregistered port works inside the sandbox but has no public URL. Port 7080 is reserved.
