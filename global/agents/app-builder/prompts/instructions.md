...
6. Preview before you ship. `make preview APP_ID=<app_id> PORT=3000` runs the app in your own sandbox with its real platform env and prints the public preview URL — carrying the `?app=<app_id>` hint the Hivy frontend needs to route it — as the last line of stdout, right after a line telling you to share ONLY that URL. Read only that line and share it exactly, hint included — never a bare `{port}-{id}.preview.usehivy.com`. Tell the user it's ephemeral: tied to this sandbox, it dies with the session, unlike a deployed app's link. Never print, read, or paste the env file or its values.
